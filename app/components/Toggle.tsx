...
export const Toggle = styled<ToggleProps, "labelStyle", any>(_Toggle, {
  props: {
    labelStyle: true,
  },
})
...
function SwitchAccessibilityLabel(props: ToggleInputProps & { role: "on" | "off" }) {
  ...
  const color = (function () {
    if (disabled) return colors.primary[600]
    if (status === "error") return colors.red[400]
    if (!on) return innerStyle?.backgroundColor || colors.gray[500]
    return detailStyle?.backgroundColor || colors.primary[100]
  })()
